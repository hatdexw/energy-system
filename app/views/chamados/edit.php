<?php
$page_title = "Editar Chamado"; // Define o título da página
ob_start(); // Inicia o output buffering
?>

<div class="bg-white shadow-md rounded-lg p-6">
    <h1 class="text-2xl font-bold mb-4 text-gray-800">Editar Chamado</h1>

    <form action="/energy-system/chamados/update" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <div class="mb-4">
                ...
                <input type="text" name="titulo" id="titulo" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars($chamado['titulo']); ?>" required>
            </div>

            <div class="mb-4">
                <label for="descricao" class="block text-gray-700 text-sm font-bold mb-2">Descrição:</label>
                <textarea name="descricao" id="descricao" rows="10" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"><?php echo htmlspecialchars($chamado['descricao']); ?></textarea>
            </div>
        ...
        <div class="lg:col-span-1 space-y-4 overflow-y-auto max-h-[80vh]">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="data_abertura" id="data_abertura" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo htmlspecialchars(date('d-m-Y H:i:s', strtotime($chamado['created_at']))); ?>" readonly>
                </div>
                <div class="mb-4">
                    <label for="tipo" class="block text-gray-700 text-sm font-bold mb-2">Tipo:</label>
                    <select name="tipo" id="tipo" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-gray-100 cursor-not-allowed" disabled>
                        <option value="Incidente" <?php echo ($chamado['tipo'] ?? 'Incidente') == 'Incidente' ? 'selected' : ''; ?>>Incidente</option>
                        <option value="Requisicao" <?php echo ($chamado['tipo'] ?? 'Incidente') == 'Requisicao' ? 'selected' : ''; ?>>Requisição</option>
                    </select>
                    <input type="hidden" name="tipo" value="<?php echo htmlspecialchars($chamado['tipo']); ?>">
                </div>
                <div class="mb-4">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                ...
                <div class="mb-4">
                    ...
                    <select name="requerente" id="requerente" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <?php foreach ($users as $user): ?>
                            <option value="<?php echo $user['id']; ?>" <?php echo ($chamado['requerente'] ?? null) == $user['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($user['full_name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="observador" class="block text-gray-700 text-sm font-bold mb-2">Observador:</label>
                    <select name="observador" id="observador" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <option value="">Nenhum</option>
                        <?php foreach ($users as $user): ?>
                            <option value="<?php echo $user['id']; ?>" <?php echo ($chamado['observador'] ?? null) == $user['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($user['full_name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="user_id" class="block text-gray-700 text-sm font-bold mb-2">Atribuído:</label>
                    <select name="user_id" id="user_id" class="shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <option value="">Nenhum</option>
                        <?php foreach ($users as $user): ?>
                            <option value="<?php echo $user['id']; ?>" <?php echo ($chamado['user_id'] == $user['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($user['full_name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
